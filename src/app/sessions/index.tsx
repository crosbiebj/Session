      ...
      {groups && groups.length > 0 ? (
        <View className="gap-2">
          ...
          <View className="flex-row flex-wrap gap-2">
            <Pressable
              onPress={() => setVisibleToGroupId(null)}
              className={`rounded-full border px-3 py-1.5 ${
                visibleToGroupId === null ? 'border-dock-amber bg-dock-amber/20' : 'border-dock-border'
              }`}
            >
              <Text
                className={`font-label text-xs uppercase tracking-wide ${
                  visibleToGroupId === null ? 'text-dock-amber' : 'text-dock-text-dim'
                }`}
              >
                Just me
              </Text>
            ...
            {groups.map((g) => (
              <Pressable
                key={g.id}
                onPress={() => setVisibleToGroupId(g.id)}
                className={`rounded-full border px-3 py-1.5 ${
                  visibleToGroupId === g.id ? 'border-dock-amber bg-dock-amber/20' : 'border-dock-border'
                }`}
              >
                <Text
                  className={`font-label text-xs uppercase tracking-wide ${
                    visibleToGroupId === g.id ? 'text-dock-amber' : 'text-dock-text-dim'
                  }`}
                >
                  {g.name}
                </Text>
